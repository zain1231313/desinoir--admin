                    ...
                    <table id="WorkTable" className="display overflow-x-scroll">
                        <thead>
                            <tr>
                                <th>Problem Image</th>
                                <th>Problem Title</th>
                                <th>Problem Title (AR)</th>
                                <th>Problem Description</th>
                                <th>Problem Description (AR)</th>
                                <th>Created at</th>
                                <th>Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            {works?.map((work) => (
                                <tr key={work._id} onClick={() => handleRowClick(work)}>
                                    <td>
                                        <Image className="h-10 w-10 rounded-full" src={work?.ProblemStatementImage} alt={work?.title?.en} width={40} height={40} />
                                    </td>
                                    {/* <td className="capitalize">{work?.types}</td> */}
                                    <td>{work?.ProblemStatementTitle?.en}</td>
                                    <td>{work?.ProblemStatementTitle?.ar}</td>
                                    <td>{work?.ProblemStatementDescription?.en?.slice(0, 20) + '...'}</td>
                                    <td>{work?.ProblemStatementDescription?.ar?.slice(0, 20) + '...'}</td>
                                    <td>{new Date(work?.createdAt).toLocaleDateString()}</td>
                                    <td>
                                        <div className="flex items-center justify-center gap-3">
                                            <button>
                                                <Link href={`/apps/our-work/edit-problems?id=${work._id}`}>
                                                    <IconPencil />
                                                </Link>
                                            </button>
